...
class DirectoryInfo:
    branches: list[tuple[str, str | None, int, int]] = (
        list()
    )  # {branch, upstream, behind, ahead}
    ...
    def has_issues(self):
        return not (
            self.has_no_uncommited_changes
...
def get_branches(directory: Path = Path()) -> list[tuple[str, str | None, int, int]]:
    ...
    try:
        ...
        if branch_result.returncode == 0:
            ...
            raise Exception(
                f"Error {branch_result.returncode} in {os.getcwd()}: {branch_result.stderr}"
            )
    finally:
        os.chdir(original)
...
def has_stash(directory: Path = Path()) -> bool:
    ...
    try:
        stash_result = subprocess.run(
            ["git", "stash", "show"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        if stash_result.returncode == 0:
            ...
            raise Exception(
                f"Error {stash_result.returncode} in {os.getcwd()}: {stash_result.stderr}"
            )
    finally:
        os.chdir(original)
...
def get_info(directory: Path = Path()) -> DirectoryInfo:
    ...
    try:
        ...
        if status_result.returncode != 0:
            raise Exception(
                f"Error {status_result.returncode} in {os.getcwd()}: {status_result.stderr}"
            )

        lines = status_result.stdout.splitlines()
        info.has_no_uncommited_changes = (
            lines[-1].decode() == "nothing to commit, working tree clean"
        )
